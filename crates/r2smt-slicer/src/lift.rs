...
impl LiftCtx {
    ...
    /// Whether a flag-setting ESIL string may lift here — now on every
    /// supported ISA when the caller enabled it.
    ///
    /// ARM was excluded for two measured reasons, both of which expired:
    /// the machine not obeying the pipeline's inverted carry convention
    /// (fixed here 2026-08-07, and locked by
    /// `test_the_arm_carry_is_stored_as_its_complement`), and radare2
    /// seeding an a64 `subs`'s flag context from the *destination* write
    /// (fixed upstream in radare2 6.2.0). It then stayed shut on inertia.
    ///
    /// Opening it was gated on its own corpus verdict diff, taken
    /// 2026-08-10 over the three ARM corpora with `--rlimit` (gate on vs
    /// `--no-esil-flags`): actionable kinds moved only in the safe
    /// direction — `arm32-fp` byte-identical, `arm32-neon` +1 and
    /// `aarch64-neon` +6 `constant_condition`/`dead_branch`, **zero
    /// actionable findings lost**. Every added verdict was spot-checked
    /// by hand and is arithmetically correct (`mov`/`add`/`cmp`/`mvn`/
    /// `csel` flag folds the per-mnemonic path left undecided). The
    /// residual ARM ESIL *value* bugs the differential harness still
    /// reports (`ldrsb` sign extension, `adc`/`sbc`, the missing
    /// `Rs[7:0]` shift mask, `fmov`'s absent zero-upper) did not surface
    /// in any actionable slice — they are wrong values, not wrong flags,
    /// and a flag branch does not read them.
    fn allows_esil_flags(&self) -> bool {
        self.esil_flags
            && matches!(
                self.arch,
                Arch::X86 | Arch::X86_64 | Arch::Arm | Arch::Aarch64
            )
    }
    ...
    fn lift_instruction(&mut self, insn: &Instruction) {
        ...
        //
        // The gate applies uniformly to every supported ISA when the
        // caller enables it (`self.esil_flags`). ARM was excluded until
        // 2026-08-10 for two measured reasons that have since expired —
        // the machine's inverted-carry-convention mismatch (fixed
        // 2026-08-07) and r2's a64 `subs` carry-seed bug (fixed upstream
        // in radare2 6.2.0) — so it is no longer special-cased here. The
        // history and the corpus verdict diff that authorised opening it
        // live on [`Self::allows_esil_flags`].

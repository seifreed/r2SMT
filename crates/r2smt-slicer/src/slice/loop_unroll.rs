...
pub(super) fn try_unroll_counted_loop(
    ...
    let init = concrete_init(function, loop_block, counter_parent, arch)?;
...
fn analyse_body(loop_block: &BasicBlock, counter_parent: &str, arch: Arch) -> Option<LoopShape> {
    ...
    for insn in &loop_block.instructions {
...
fn concrete_init(
    ...
    let entry = preds.into_iter().find(|p| *p != loop_block.address)?;

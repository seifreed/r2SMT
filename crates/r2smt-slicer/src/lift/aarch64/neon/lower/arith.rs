...
///
/// Rounding adds half an ulp of the shift (`2^(amount-1)`) *before* the
/// low bits are discarded, and at the element's own width that addition
/// can carry out — `0xff + 1` in eight bits is zero. The sum is therefore
/// taken in a wider intermediate that reproduces ARM's unbounded-integer
/// definition.
///
/// `max_shift` is the largest amount the caller can pass. It sizes the
/// intermediate: the rounding term `2^(amount-1)` must sit strictly below
/// the sign bit, or a signed `ashr` reads a positive sum as negative and
/// smears sign bits back into the result. An immediate amount is bounded
/// by the element (`max_shift <= lane_bits`, so the intermediate stays
/// `lane_bits + 1`); the register forms read a signed byte whose right
/// shift reaches 128, well past a narrow lane, so they need the extra
/// room.
fn shift_right(
    value: Expr,
    amount: &Expr,
    ...
    let wide = lane_bits.max(max_shift).checked_add(1)?;
...
/// The largest right-shift magnitude a register shift amount can name: the
/// low byte read as a signed value ranges to `-128`, so the shift reaches
/// `128` regardless of the element width.
const REGISTER_SHIFT_MAX: u16 = 1 << (BITS_PER_BYTE - 1);

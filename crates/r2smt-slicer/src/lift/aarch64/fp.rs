...
use super::neon;
use super::neon::lower::convert_lane;
use super::neon::width::ConvertKind;
use super::sign_bit_mask;

/// The fixed-point fraction width of a `scvtf` / `ucvtf` / `fcvtzs` /
/// `fcvtzu`, or `Some(0)` for the plain integer↔float register form.
///
/// The fixed-point forms carry an immediate `#fbits` (ARM ARM C7.2.—)
/// that scales the value: `scvtf Dd, Xn, #fbits` reads `Xn` as
/// `Xn / 2^fbits`, and `fcvtzs Wd, Sn, #fbits` truncates `Sn * 2^fbits`.
/// Dropping it lifts the fixed-point form as if it were the plain one — a
/// wrong value. `fixed_bits` is the integer (fixed-point) side's width,
/// which bounds the immediate to `1..=fixed_bits`; an out-of-range or
/// unparseable amount is not an instruction, so it returns `None` (the
/// caller declines). Mirrors the `AArch32` VFP path's
/// `vfp_convert_fraction_bits`.
fn aarch64_fixed_fraction_bits(insn: &Instruction, fixed_bits: u16) -> Option<u16> {
...
impl LiftCtx {
    ...
    fn lift_aarch64_int_to_fp(&mut self, insn: &Instruction, signed: bool) {
        ...
        // The fixed-point form (`scvtf Dd, Xn, #fbits`) reads the integer
        // as `Xn / 2^fbits`; the immediate is bounded by the integer
        // side's width. The plain form has no third operand → `fbits = 0`
        // → byte-identical to the prior conversion. `convert_lane` handles
        // the signed / unsigned extension (the IR carries only the signed
        // node) and the scaling.
        let Some(fbits) = aarch64_fixed_fraction_bits(insn, source_bits) else {
            self.push_aarch64_fp_unsupported(insn);
            return;
        };
        let converted = convert_lane(
        ...
        if !self.write_simd_dst(dst, converted, true) {
            self.push_aarch64_fp_unsupported(insn);
        }
    }
    ...
    fn lift_aarch64_fp_to_int(&mut self, insn: &Instruction, signed: bool) {
        ...
        // The fixed-point form (`fcvtzs Wd, Sn, #fbits`) truncates
        // `Sn * 2^fbits`; the immediate is bounded by the integer
        // (destination) side's width. Plain form → `fbits = 0` →
        // byte-identical to the prior conversion. `convert_lane` handles
        // the extra-bit unsigned-range trick (the IR has no unsigned
        // float-to-integer node) and the scaling.
        let Some(fbits) = aarch64_fixed_fraction_bits(insn, width) else {
            self.push_aarch64_fp_unsupported(insn);
            return;
        };
        let converted = convert_lane(

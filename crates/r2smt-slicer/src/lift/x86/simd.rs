...
use super::{
    ABSOLUTE, ADD, AVERAGE, BITS_PER_BYTE, F16C_HALF_BITS, F16C_IMM_ROUNDING_MASK,
    F16C_IMM_USE_MXCSR, F16C_SINGLE_BITS, FpCompare, MAX_SIGNED, MAX_UNSIGNED, MIN_SIGNED,
    MIN_UNSIGNED, MUL, MUL_HIGH_SIGNED, MUL_HIGH_UNSIGNED, SAT_ADD_SIGNED, SAT_ADD_UNSIGNED,
    SAT_SUB_SIGNED, SAT_SUB_UNSIGNED, SUB, SimdBitOp, VECTOR_TEST_CLEARED_FLAGS, fp_mask_lane,
    is_vex, parse_immediate, same_xmm_register,
};
...
impl LiftCtx {
    ...
    fn vex_scalar_move_value(
        ...
        self.vex_merge_scalar(dst, src1, low, lane)
    }

    /// Merge a computed `low` lane into `src1`'s upper lanes at `dst`'s
    /// view width, for a VEX 3-operand scalar form: `dst[lane-1:0] = low`,
    /// `dst[view-1:lane] = src1[view-1:lane]`, and the caller's
    /// `write_simd_dst(.., true)` zeroes `dst[MAXVL-1:view]`.
    fn vex_merge_scalar(
        &mut self,
        dst: &Operand,
        src1: &Operand,
    ...
    /// *within the destination's view* rather than merging: `movd xmm0,
    /// eax` clears bits 127:32 of the XMM register, and the
    /// general-register direction zero-extends into the full register.
    /// Merging the low bits instead would leave whatever the view held
    /// before, which is a wrong value rather than a decline. Above the
    /// 128-bit view the legacy SSE form **preserves** the parent
    /// (`MAXVL-1:128` unmodified per the SDM); only the VEX form zeroes
    /// it — the same `zero_upper = is_vex` rule the whole SIMD lifter uses.
    pub(super) fn lift_simd_gpr_transfer(&mut self, insn: &Instruction, width: u16) {
        ...
        if to_vector {
            let Some(view) = self.simd_view_bits(&dst) else {
                self.push_simd_unsupported(insn);
                return;
            };
            // Size the value to the destination's view, zeroing the bits
            // above the transferred width *inside* the view (`movd` clears
            // [view-1:32]). Above the view, only the VEX form zeroes the
            // parent; the legacy SSE form preserves [MAXVL-1:128].
            let view_value = if view > width {
                Expr::zero_ext(value, view)
            } else {
                value
            };
            if !self.write_simd_dst(&dst, view_value, is_vex(insn)) {
                self.push_simd_unsupported(insn);
            }
            return;
        }
    ...
    pub(super) fn lift_f16c_widen(&mut self, insn: &Instruction) {
        ...
        // Widening f16 → f32 is exact, so the rounding mode is
        // immaterial; pass the architectural default.
        let Some(result) = self.f16c_lanes(
            dst,
            src,
            F16C_HALF_BITS,
            F16C_SINGLE_BITS,
    ...
    ///
    /// The immediate selects the rounding mode: bit 2 means "use MXCSR"
    /// — which this lifter cannot pin, so that encoding declines rather
    /// than assuming a mode — and otherwise bits `[1:0]` name the mode
    /// directly (0 nearest-even, 1 down, 2 up, 3 toward-zero). Narrowing
    /// f32 → f16 actually rounds, so the mode is a wrong *value* if
    /// ignored, not a lost one.
    pub(super) fn lift_f16c_narrow(&mut self, insn: &Instruction) {
        let (Some(dst), Some(src)) = (insn.operands.first(), insn.operands.get(1)) else {
            return;
        };
        let Some(imm) = insn.operands.get(2).and_then(|o| parse_immediate(&o.raw)) else {
            self.push_simd_unsupported(insn);
            return;
        };
        if imm & F16C_IMM_USE_MXCSR != 0 {
            self.push_simd_unsupported(insn);
            return;
        }
        let rm = match imm & F16C_IMM_ROUNDING_MASK {
            0 => RoundingMode::NearestTiesEven,
            1 => RoundingMode::TowardNegative,
            2 => RoundingMode::TowardPositive,
            _ => RoundingMode::TowardZero,
        };
        let Some(result) = self.f16c_lanes(src, src, F16C_SINGLE_BITS, F16C_HALF_BITS, rm) else {
            self.push_simd_unsupported(insn);
            return;
        };
        if !self.write_simd_dst(dst, result, true) {
    ...
    /// Convert every lane of `src` from `from_bits` to `to_bits` under
    /// `rm`, with the lane count taken from `count_from`'s view divided
    /// by the wider of the two lane widths.
    fn f16c_lanes(
        &mut self,
        count_from: &Operand,
        src: &Operand,
    ...
    pub(super) fn lift_simd_fp_mask_compare(&mut self, insn: &Instruction, cmp: &FpCompare) {
        ...
        } else if ops.len() == 3 && is_vex(insn) {
            // VEX scalar (`vcmpeqss`/`vcmpeqsd`): low lane = mask, upper
            // lanes from src1 (`a_op`), and the register above the view is
            // zeroed. The legacy 2-operand form below merges into `dst`.
            match self.vex_merge_scalar(dst, a_op, result, cmp.lane_bits) {
                Some(merged) => self.write_simd_dst(dst, merged, true),
                None => false,
            }
        } else {

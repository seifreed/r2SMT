...
pub(in crate::lift::aarch32) use vfp::vfp_convert;
pub(crate) use vfp::{VfpOp, vfp_scalar};
pub(super) mod neon;

use super::{
    BinOp, FpArithOp, LiftCtx, MemAccess, PackedIntOp, PackedOp, VectorShape, Writeback,
    aarch32_predicable_base, aarch64_cond_suffix_to_predicate, constant_delta,
...
impl LiftCtx {
    ...
    fn lift_aarch32_by_mnemonic(&mut self, insn: &Instruction, mnem: &str) {
        match mnem {
            ...
            // The whole shift family goes through one handler, because
            // what separates it from the arithmetic one is the flags and
            // the amount rather than the operation.
            "lsl" => self.lift_aarch32_shift(insn, ShiftForm::By(BinOp::Shl), false),
            "lsls" => self.lift_aarch32_shift(insn, ShiftForm::By(BinOp::Shl), true),
            "lsr" => self.lift_aarch32_shift(insn, ShiftForm::By(BinOp::Shr), false),
            "lsrs" => self.lift_aarch32_shift(insn, ShiftForm::By(BinOp::Shr), true),
            "asr" => self.lift_aarch32_shift(insn, ShiftForm::By(BinOp::Sar), false),
            "asrs" => self.lift_aarch32_shift(insn, ShiftForm::By(BinOp::Sar), true),
            "ror" => self.lift_aarch32_shift(insn, ShiftForm::By(BinOp::Ror), false),
            "rors" => self.lift_aarch32_shift(insn, ShiftForm::By(BinOp::Ror), true),
            "rrx" => self.lift_aarch32_shift(insn, ShiftForm::ThroughCarry, false),
            "rrxs" => self.lift_aarch32_shift(insn, ShiftForm::ThroughCarry, true),
    ...
    fn lift_aarch32_shift(&mut self, insn: &Instruction, form: ShiftForm, sets_flags: bool) {
...
/// same reason [`Aarch32Shift`] splits it out: it takes no amount and it
/// is not a function of the register alone.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// by the addressing model and the standalone mnemonic so the two
/// spellings of one operation cannot drift apart.
fn rotate_right_through_carry(value: Expr, bits: u16) -> Expr {
        ...
        MemOffset::Register {
            ...
            match shift {
                ...
                Some(Aarch32Shift::ThroughCarry) => {
                    value = rotate_right_through_carry(value, ptr_bits);
                }
            }
            if *subtract {

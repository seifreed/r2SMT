...
/// Clamp an unsigned float-to-integer conversion at its low end.
///
/// The IR has no unsigned float-to-integer node, so the unsigned forms
/// go through the signed `fp.to_sbv` with one extra bit of range — which
/// covers the whole unsigned interval for a non-negative source, but
/// faithfully wraps a *negative* one to its two's-complement pattern
/// (`-1.0` → all-ones → the unsigned maximum). ARM `FCVTZU` / `FCVTxU`
/// instead saturate a NaN or negative source to `0`. `value` is the
/// source float in sort `(ebits, sbits)`, `width` the destination
/// integer width, and `in_range` the truncated result for a
/// non-negative source. (Positive overflow is left to `fp.to_sbv`, the
/// same as the signed forms.)
pub(in crate::lift) fn clamp_unsigned_float_to_int(
    value: Expr,
    ebits: u16,
    sbits: u16,
    width: u16,
    ...
    let zero = Expr::FpConst {
        bits: 0,
        ebits,
        sbits,
    };
    let negative_or_nan = Expr::bool_or(Expr::flt(value.clone(), zero), Expr::fisnan(value));
    Expr::Ite {
        cond: Box::new(negative_or_nan),
        then_expr: Box::new(Expr::konst(0, width)),
        else_expr: Box::new(in_range),
    }
}

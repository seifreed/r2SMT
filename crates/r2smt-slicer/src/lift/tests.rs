...
#[test]
fn aarch64_narrowing_2_forms_read_their_destination() {
    // The narrowing `2` forms write only the destination's upper half and
    // preserve its lower half, so their lowering reads the destination.
    // The effect table must add it to `uses`, or the backward walk drops
    // whatever produced the low half and SSA binds it to a free input.
    let cases: &[(&str, &[&str])] = &[
        ("sqxtn2", &["v0.16b", "v1.8h"]),
        ("uqxtn2", &["v0.16b", "v1.8h"]),
    ...
    for (mnem, ops) in cases {
        let operands: Vec<Operand> = ops
            .iter()
            .map(|o| {
                let kind = if o.starts_with('v') {
            ...
            .collect();
        let i = insn(0x1000, 4, mnem, operands);
        let e = crate::effect::analyze(&i, Arch::Aarch64);
        assert!(e.defs.contains(&"v0"), "{mnem}: defs {e:?}");
        assert!(
            e.uses.contains(&"v0"),
            "{mnem}: must read its destination {e:?}"
        );
    }
    // The widening `fcvtl2` writes the whole destination and must NOT be
    // marked as reading it.
    let widen = insn(
        0x1000,
        4,
        "fcvtl2",
    ...
    let e = crate::effect::analyze(&widen, Arch::Aarch64);
    assert!(
        !e.uses.contains(&"v0"),
        "fcvtl2 must not read its destination {e:?}"
    );
}

#[test]
fn aarch32_rrx_indexed_store_reads_the_carry() {
    // A store computes its address exactly as a load does, so an `rrx`
    // index makes the store address depend on `CF` too. The effect table
    // used to hardcode `reads_flags: false` for `str`, the lone outlier
    // among the AArch32 memory effects — which drops the `CF` definer and
    // lets SSA bind the address's carry to a stale free input: a
    // fabricated verdict for the aliasing read.
    for mnemonic in ["str", "strb", "strh"] {
        let insn = insn(
            0x1000,
            4,
            mnemonic,
        ...
        assert!(
            crate::effect::analyze(&insn, Arch::Arm).reads_flags,
            "{mnemonic}: an rrx-indexed store reads CF"
        );
    }
    // A plain index shift on a store still does not read the carry.
    let plain = insn(
        0x1000,
        4,
        "str",
...
#[test]
fn movd_to_xmm_is_legacy_and_preserves_upper_parent_bits() {
    // `movd xmm0, eax` is a legacy SSE write: it zeroes [127:32] inside
    // the view but must PRESERVE [MAXVL-1:128]. Zeroing the whole 512-bit
    // parent (the VEX rule) would fabricate an all-zero ymm0/zmm0 that a
    // later `vptest ymm0, ymm0` reads as definitely zero.
    let stmts = lift_per_mnemonic(
        &insn(
            0x1000,
            4,
    ...
    let Expr::Concat { high, low } = simd_dst_src(&stmts, "zmm0") else {
        panic!("legacy movd must reconstruct the parent by preserving its upper bits");
    };
    assert_eq!(
        **high,
        Expr::extract(Expr::var("zmm0", 512), 511, 128),
        "the 384 bits above the view must be preserved"
    );
    assert!(
        matches!(**low, Expr::ZeroExtend { to_bits: 128, .. }),
        "the low 128 bits are the transferred value zero-extended: {low:?}"
    );
}

#[test]
fn vmovd_to_xmm_zeroes_the_upper_parent_bits() {
    // The VEX form zeroes [MAXVL-1:128], unlike its legacy twin above.
    let stmts = lift_per_mnemonic(
        &insn(
            0x1000,
            4,
    ...
    assert!(
        matches!(
            simd_dst_src(&stmts, "zmm0"),
            Expr::ZeroExtend { to_bits: 512, .. }
        ),
        "VEX vmovd must zero the parent above the view: {:?}",
        simd_dst_src(&stmts, "zmm0")
    );
}
...
#[test]
fn vex_scalar_compare_takes_upper_lanes_from_src1_and_zeroes_above_the_view() {
    // `vcmpeqss xmm0, xmm1, xmm2` — the low lane is the compare mask,
    // the rest of the 128-bit view comes from src1 (xmm1), and the VEX
    // encoding zeroes everything above the view. The legacy 2-operand
    // form instead merges into the destination's own prior contents, so
    // taking the upper lanes from `xmm0` here would be a wrong value.
    let stmts = lift_per_mnemonic(
        &insn(
            0x1000,
            4,
    ...
    let Expr::ZeroExtend {
        ...
        panic!("expected a VEX zero-extend to the full register, got {src:?}");
    };
    assert_eq!(*to_bits, 512, "the VEX form zeroes above the 128-bit view");
    let Expr::Concat { high, .. } = &**inner else {
        panic!("expected concat(upper, low), got {inner:?}");
    };
    assert_eq!(
        **high,
        Expr::extract(simd_read("zmm1", 128), 127, 32),
        "the upper lanes must come from src1 (xmm1), not the destination"
    );
}
...
#[test]
fn vcvtps2ph_narrowing_uses_the_immediate_rounding_mode() {
    // Narrowing f32 → f16 actually rounds, so when the immediate names an
    // explicit mode (bit 2 clear) the lifter must convert under it, not
    // under the hardcoded nearest-even. imm 3 = toward zero, imm 1 = down.
    for (imm, expected) in [("3", "TowardZero"), ("1", "TowardNegative")] {
        let stmts = lift_per_mnemonic(
            &insn(
                0x1000,
                5,
        ...
        assert!(
            ...
            "imm {imm} should round {expected}: {rendered}"
        );
    }
}
...
#[test]
fn aarch64_scvtf_fixed_point_applies_the_fraction_immediate() {
    // `scvtf s0, w0, #4` reads w0 as a fixed-point value with 4
    // fractional bits (`w0 / 2^4`). Dropping the immediate lifts it as
    // the plain integer conversion — a wrong value, off by 2^4.
    let plain = lift_aarch64("scvtf", vec![reg("s0"), reg("w0")]);
    let fixed = lift_aarch64(
        "scvtf",
        vec![reg("s0"), reg("w0"), op("4", OperandKind::Immediate)],
    );
    assert_ne!(plain, fixed, "the #fbits immediate must change the result");
    let rendered = format!("{:?}", simd_parent_src(&fixed, "v0"));
    assert!(
        rendered.contains("FMul"),
        "scvtf #fbits must scale by 2^-fbits: {rendered}"
    );
    assert!(
        !fixed
            .iter()
            .any(|s| matches!(s, IrStmt::Unsupported { .. })),
        "the fixed-point form must lift, not decline: {fixed:?}"
    );
    // The immediate is bounded by the integer side's width (w0 → 32), so
    // an out-of-range fraction is not an instruction and declines.
    let bad = lift_aarch64(
        "scvtf",
        vec![reg("s0"), reg("w0"), op("33", OperandKind::Immediate)],
    );
    assert!(
        bad.iter().any(|s| matches!(s, IrStmt::Unsupported { .. })),
        "fbits past the integer width must decline: {bad:?}"
    );
}

#[test]
fn aarch64_fcvtzs_fixed_point_applies_the_fraction_immediate() {
    // `fcvtzs w0, s0, #4` truncates `s0 * 2^4`; dropping the immediate is
    // a wrong value.
    let plain = lift_aarch64("fcvtzs", vec![reg("w0"), reg("s0")]);
    let fixed = lift_aarch64(
        "fcvtzs",
        vec![reg("w0"), reg("s0"), op("4", OperandKind::Immediate)],
    );
    assert_ne!(plain, fixed, "the #fbits immediate must change the result");
    assert!(
        !fixed

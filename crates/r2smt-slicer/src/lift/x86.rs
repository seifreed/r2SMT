...
impl LiftCtx {
    ...
    fn lift_add_sub(&mut self, insn: &Instruction, is_add: bool) {
        ...
        let lhs_read = self.read_operand_lowered(dst, dst_width);
        let rhs_read = self.read_operand_lowered(src, dst_width);
        // Stash both operand values and the result in fresh temps before
        // the destination write. SSA renames reads by statement position,
        // so a flag expression that read the destination register
        // directly would bind to the *post-write* version; the temps make
        // every flag input immune to that. It matters here and not just
        // for the result because CF reads an operand: the `add` form is
        // `result <u dst`, and any `dst == src` form reads a source the
        // write clobbers.
        let lhs_tmp = self.new_temp(insn.address, dst_width);
        self.assign(lhs_tmp.clone(), lhs_read);
        let rhs_tmp = self.new_temp(insn.address, dst_width);
        self.assign(rhs_tmp.clone(), rhs_read);
        let lhs = Expr::Var(lhs_tmp);
        let rhs = Expr::Var(rhs_tmp);
        let result_tmp = self.new_temp(insn.address, dst_width);
        let computed = if is_add {
            Expr::add(lhs.clone(), rhs.clone())
        } else {
            Expr::sub(lhs.clone(), rhs.clone())
        };
        self.assign(result_tmp.clone(), computed);
        let result = Expr::Var(result_tmp);
        if !self.write_register_to(dst, result.clone()) {
            self.stmts.push(IrStmt::Unsupported {
                mnemonic: insn.mnemonic.clone(),
                comment: "non-register destination".into(),
            });
        ...
        self.set_flag("ZF", Expr::eq(result.clone(), Expr::konst(0, dst_width)));
        self.set_flag("SF", Expr::slt(result.clone(), Expr::konst(0, dst_width)));
        // CF: `add` carries out when the unsigned result wraps below the
        // original (`result <u lhs`); `sub` borrows when the minuend is
        // below the subtrahend (`lhs <u rhs`).
        self.set_flag(
            "CF",
            if is_add {
                Expr::ult(result.clone(), lhs.clone())
            } else {
                Expr::ult(lhs.clone(), rhs.clone())
            },
        );
        self.set_flag(
            "OF",
            if is_add {
                x86_add_overflow(lhs, rhs, result.clone(), dst_width)
            } else {
                x86_sub_overflow(lhs, rhs, result.clone(), dst_width)
            },
        );
        self.set_flag("PF", x86_parity(&result, dst_width));
    }
    ...
    fn lift_cmp(&mut self, insn: &Instruction) {
        ...
        self.set_flag("SF", Expr::slt(tmp_expr.clone(), Expr::konst(0, cmp_width)));
        // For `cmp lhs, rhs`, CF is the unsigned borrow `lhs <u rhs` and
        // OF is the signed overflow of the same subtraction.
        self.set_flag("CF", Expr::ult(lhs.clone(), rhs.clone()));
        self.set_flag(
            "OF",
            x86_sub_overflow(lhs, rhs, tmp_expr.clone(), cmp_width),
        );
        self.set_flag("PF", x86_parity(&tmp_expr, cmp_width));
    }
...
/// Signed-overflow bit of `a - b == result` at `width`: the two source
/// signs differing while the result's sign differs from the minuend's,
/// read off the top bit of `(a ^ b) & (a ^ result)`.
fn x86_sub_overflow(a: Expr, b: Expr, result: Expr, width: u16) -> Expr {
    let msb = width - 1;
    Expr::extract(
        Expr::bv_and(Expr::bv_xor(a.clone(), b), Expr::bv_xor(a, result)),
        msb,
        msb,
    )
}
...
/// Signed-overflow bit of `a + b == result` at `width`: the two source
/// signs agreeing while the result's sign differs from them, read off
/// the top bit of `(a ^ result) & (b ^ result)`.
fn x86_add_overflow(a: Expr, b: Expr, result: Expr, width: u16) -> Expr {
    let msb = width - 1;
    Expr::extract(
        Expr::bv_and(Expr::bv_xor(a, result.clone()), Expr::bv_xor(b, result)),
        msb,
        msb,
    )
}

/// x86 parity is computed over the low byte of the result only.
const X86_PARITY_BITS: u16 = 8;

/// `PF`: even parity of the result's low byte. The flag is set when the
/// count of set bits among the low `min(8, width)` is even, which is the
/// complement of their exclusive or.
fn x86_parity(value: &Expr, width: u16) -> Expr {
    let top = width.min(X86_PARITY_BITS);
    let fold = (0..top).fold(None, |acc: Option<Expr>, i| {
        let bit = Expr::extract(value.clone(), i, i);
        Some(match acc {
            None => bit,
            Some(prev) => Expr::bv_xor(prev, bit),
        })
    });
    let xor = fold.unwrap_or_else(|| Expr::konst(0, 1));
    Expr::bv_xor(xor, Expr::konst(1, 1))
}
...
/// `vcvtps2ph` immediate bits `[1:0]` naming the explicit rounding mode
/// when [`F16C_IMM_USE_MXCSR`] is clear.
const F16C_IMM_ROUNDING_MASK: u64 = 0b11;

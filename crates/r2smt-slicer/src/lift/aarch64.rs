...
use super::{
    BinOp, CsArithOp, LiftCtx, MemAccess, VectorShape, Writeback, aarch64_cond_suffix_to_predicate,
    constant_delta, nonzero_width, vector_shape, width_mask,
};
...
impl LiftCtx {
    pub(super) fn lift_instruction_aarch64(&mut self, insn: &Instruction) {
        ...
        match mnem.as_str() {
            ...
            // first. The pair goes through `aarch64_pair_addresses`,
            // which shares the single-register `aarch64_mem_access`
            // parser, so the pre-index (`[Xn, #imm]!`) and post-index
            // (`[Xn], #imm`) writeback forms advance the base like the
            // single-register loads do.
    ...
    pub(super) fn lift_aarch64_ldp(&mut self, insn: &Instruction) {
        ...
        let Some((first, second, writeback)) =
            self.aarch64_pair_addresses(mem, insn.operands.get(3), w0)
        else {
            self.stmts.push(IrStmt::Unsupported {
                mnemonic: insn.mnemonic.clone(),
    ...
    /// Addresses of the two paired elements plus the base writeback:
    /// the first element's address, the base plus one element
    /// (`first_width / 8` bytes), and the `Xn := Xn ± imm` update for a
    /// pre / post-index form (`None` for a plain offset access). Shares
    /// the single-register `aarch64_mem_access` parser so the pair honours
    /// the same writeback forms; register-offset shapes still decline.
    fn aarch64_pair_addresses(
        &self,
        mem: &Operand,
        post: Option<&Operand>,
        first_width: u16,
    ) -> Option<(Expr, Expr, Option<Writeback>)> {
        let access = aarch64_mem_access(mem, post, self.bits)?;
        let stride = u64::from(first_width / 8) & width_mask(self.bits);
        let second = Expr::add(
            access.address.clone(),
            Expr::konst(u128::from(stride), self.bits),
        );
        Some((access.address, second, access.writeback))
    }
}
...
/// is zero). The `offset` is reinterpreted as its two's-complement
/// `u64` pattern so a negative displacement folds correctly at
/// `ptr_bits` under `bvadd`.

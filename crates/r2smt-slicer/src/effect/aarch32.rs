...
fn aarch32_str_effect(insn: &Instruction) -> InstructionEffect {
    ...
    InstructionEffect {
        ...
        has_memory_access: true,
        is_call: false,
        // A store's address is computed the same way a load's is, so an
        // `rrx` index shift makes it depend on `CF` exactly as in
        // `aarch32_ldr_effect`. Hardcoding `false` here would drop the
        // `CF` definer from the slice and let SSA bind the store
        // address's carry to a stale free input — a fabricated verdict.
        reads_flags: aarch32_memory_reads_carry(insn),
    }
}

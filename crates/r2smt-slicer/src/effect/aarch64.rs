...
/// The base register inside an `AArch64` memory operand (first register
/// named in the brackets), ignoring a trailing pre-index `!`.
fn aarch64_memory_base(mem: &Operand) -> Option<&'static str> {
    let raw = mem.raw.trim();
    let body = raw.strip_suffix('!').unwrap_or(raw);
    let body = body.strip_prefix('[')?.strip_suffix(']')?;
    canonical_register(body.split(',').next()?.trim(), Arch::Aarch64)
}

/// The base register a memory access writes back to, for a pre-index
/// (`[Xn, #imm]!`) or post-index (`[Xn], #imm`) form.
///
/// The lifter emits `Xn := Xn ± delta` for both, so the effect table
/// must report `Xn` as a definition. Without it the walk steps past this
/// instruction while `Xn` is live, binds the read to an earlier
/// definition, and the slice carries a base value the machine never held
/// (a fabricated verdict). `memory_index` is where the bracketed operand
/// sits — 1 for `ldr`/`str`, 2 for `ldp`/`stp`.
fn aarch64_writeback_base_at(insn: &Instruction, memory_index: usize) -> Option<&'static str> {
    let mem = insn.operands.get(memory_index)?;
    if !mem.raw.trim().ends_with('!') && insn.operands.len() <= memory_index + 1 {
        return None;
    }
    aarch64_memory_base(mem)
}
...
fn aarch64_ldr_effect(insn: &Instruction) -> InstructionEffect {
    ...
    if let Some(base) = aarch64_writeback_base_at(insn, 1)
        && !defs.contains(&base)
    {
        defs.push(base);
    }
...
fn aarch64_str_effect(insn: &Instruction) -> InstructionEffect {
    ...
    // A pre / post-index `str` advances its base; that write must be a
    // definition or the slicer binds a later base read to a stale value.
    let defs: Vec<&'static str> = aarch64_writeback_base_at(insn, 1).into_iter().collect();
    InstructionEffect {
        // `Mov` keeps the slicer out of the `Other`-truncation path
...
fn aarch64_ldp_effect(insn: &Instruction) -> InstructionEffect {
    ...
    if let Some(base) = aarch64_writeback_base_at(insn, 2)
        && !defs.contains(&base)
    {
        defs.push(base);
    }
...
fn aarch64_stp_effect(insn: &Instruction) -> InstructionEffect {
    ...
    // A pre / post-index `stp` advances its base; that write must be a
    // definition so a later base read is not bound to the stale value.
    let defs: Vec<&'static str> = aarch64_writeback_base_at(insn, 2).into_iter().collect();
    InstructionEffect {
        kind: InstructionKind::Mov,
        defs,
        uses,

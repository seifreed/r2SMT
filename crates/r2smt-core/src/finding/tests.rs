...
#[test]
fn a_slice_with_an_unsupported_statement_is_not_actionable() {
    // An `IrStmt::Unsupported` is a dropped instruction: its register
    // keeps a stale value instead of being havocked, so a definite
    // verdict can be fabricated in the `AlwaysX` direction. Even a
    // Complete slice with no inputs and an `AlwaysTrue` verdict — which
    // would otherwise be a High-confidence `ConstantCondition` — must be
    // reported as non-actionable `SuspiciousButUnknown` at `Unknown`
    // confidence, exactly like a truncated / unsound slice.
    let program = one_block(vec![
        insn(
            0x40_1000,
            2,
    ...
    let base = ssa_first(&program);
    let ssa = SsaLiftedSlice {
        statements: vec![IrStmt::Unsupported {
            mnemonic: "weird".into(),
            comment: "no lifter handler".into(),
        }],
...
fn complete_slice_with_unknowns_yields_medium_confidence() {
    // A shift still emits CF/OF/PF as Unknown (its count-dependent flags
    // are not modelled), so its slice carries Unknown exprs while ZF —
    // which `je` reads — stays precise. `cmp`/`add`/`sub` no longer serve
    // here: their flags are now modelled exactly.
    let program = one_block(vec![
        insn(
            0x40_1000,
            3,
...
fn signed_jge_with_unknown_flags_downgrades_to_low_confidence() {
    // shl eax, 1; jge dest — `jge` depends on `SF == OF`, and a shift
    // still leaves `OF` Unknown (its count-dependent flags are not
    // modelled). With a free OF the solver may resolve either way, so
    // the verdict does not deserve more than `Low`.
    let program = one_block(vec![
        insn(
            0x40_1000,
            3,
    ...
    let ssa = ssa_first(&program);
    let finding = classify_finding(&ssa, SmtResult::BothPossible);
    assert!(finding.evidence.unknown_count > 0);
    assert_eq!(finding.confidence, Confidence::Low);
}

#[test]
fn signed_jge_off_a_modelled_compare_keeps_its_confidence() {
    // cmp eax, 0; jge dest — the compare now models `OF` (and `PF`)
    // exactly, so the slice carries no Unknown and the signed-branch
    // downgrade must not fire: a definitive verdict keeps `High`.
    let program = one_block(vec![
        insn(
            0x40_1000,
            3,
    ...
    let ssa = ssa_first(&program);
    let finding = classify_finding(&ssa, SmtResult::AlwaysFalse);
    assert_eq!(finding.evidence.unknown_count, 0);

...
///
/// The verdict is [`SmtResult::AlwaysTrue`] when the resolved target
/// equals [`BranchCandidate::taken_target`] and
/// [`SmtResult::AlwaysFalse`] when it equals
/// [`BranchCandidate::fallthrough_target`]; on those two matches the
/// finding is [`FindingKind::ConstantCondition`] at [`Confidence::High`],
/// reflecting that the CFG analyser is authoritative. When the resolved
/// target matches neither, this returns [`None`] rather than guessing a
/// verdict — the disagreement between r2's CFG and the operand decoder is
/// surfaced to the SMT pipeline instead.
#[must_use]
...
pub fn classify_finding_with_hints(
    ...
    // An `IrStmt::Unsupported` is a total modeling hole: the lifter could
    // not lower the instruction, so it — and the register it would have
    // defined — is dropped from the formula. The def keeps its stale
    // in-slice value instead of being havocked, which can turn a real
    // `BothPossible` into a fabricated `AlwaysX`. Unlike an `Expr::Unknown`
    // (a free value the solver ranges over, which only ever weakens a
    // verdict), this is unsound in the fabricating direction, so counting
    // it as an unknown — which would only drop the confidence to Medium —
    // is not enough. Treat the slice like a truncated / unsound one:
    // non-actionable `SuspiciousButUnknown` at `Unknown` confidence.
    let has_modeling_hole = slice
        .statements
        .iter()
        .any(|s| matches!(s, IrStmt::Unsupported { .. }));
    let is_complete = matches!(slice.status, SliceStatus::Complete);
    let kind = if has_modeling_hole {
        FindingKind::SuspiciousButUnknown
    } else {
        kind_for(verdict, slice)
    };
    let raw_confidence = if has_modeling_hole {
        Confidence::Unknown
    } else {
        confidence_for(
            verdict,
    ...
    let confidence =
        downgrade_for_unmodeled_flags(raw_confidence, slice.branch.condition, unknown_count);
...
/// branch's flag predicate references flags the lifter left unmodelled
/// (`OF` / `PF`) *and this slice actually carries an Unknown*. Signed
/// comparisons (`jg`, `jl`, …) and parity / overflow branches can read
/// those flags, so when one is free the verdict could only be
/// coincidental with the Unknown the solver picked and does not deserve
/// `High` / `Medium` trust.
///
/// The `unknown_count` gate is what keeps this from over-firing now that
/// the per-mnemonic x86 handlers model `CF` / `OF` / `PF` exactly for
/// `add` / `sub` / `cmp`: a signed branch off a modelled compare has no
/// Unknown, so it keeps the trust `confidence_for` already assigned.
/// A slice that still holds an Unknown (e.g. an upstream shift, whose
/// count-dependent flags are unmodelled) stays downgraded, exactly as
/// before — this is a strict relaxation, never a new downgrade.
fn downgrade_for_unmodeled_flags(
    confidence: Confidence,
    condition: BranchCondition,
    unknown_count: usize,
) -> Confidence {
    if unknown_count > 0
        && condition.depends_on_unmodeled_flag()

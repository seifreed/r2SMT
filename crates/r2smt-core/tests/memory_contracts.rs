...
#[test]
fn test_aarch64_postindex_ldp_emits_loads_and_base_writeback() {
    // `ldp x29, x30, [sp], #0x10` — four operands: load the pair at sp
    // and sp+8, then sp := sp + 0x10. The writeback must not be dropped.
    let stmts = lift_per_mnemonic(
        &insn(
            0x1000,
            "ldp",
    ...
    assert_eq!(
        stmts
            ...
            .count(),
        2,
        "ldp must load both registers: {stmts:?}"
    );
    assert!(
        has_writeback_to(&stmts, "sp"),
        "post-index ldp must advance sp: {stmts:?}"
    );
}

#[test]
fn test_aarch64_postindex_stp_emits_stores_and_base_writeback() {
    // `stp x29, x30, [sp], #-0x10` — store the pair, then sp := sp - 0x10.
    let stmts = lift_per_mnemonic(
        &insn(
            0x1000,
            "stp",
    ...
    assert_eq!(
        stmts
            ...
            .count(),
        2,
        "stp must store both registers: {stmts:?}"
    );
    assert!(
        has_writeback_to(&stmts, "sp"),
        "post-index stp must advance sp: {stmts:?}"
    );
}

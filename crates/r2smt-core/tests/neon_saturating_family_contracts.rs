...
#[test]
fn srshl_rounds_a_large_negative_register_amount_to_zero() {
    // A register amount of -9 (0xf7) right-shifts by 9, one past the
    // element width. The half-ulp `1 << 8` reaches the sign bit of a
    // nine-bit intermediate, so the signed shift read a positive sum as
    // negative and answered 0xff. Sized for the register form's reach
    // (a signed byte, up to 128) the rounding of 127 / 2^9 is 0.
    assert_computes(
        "srshl",
        &["v0.8b", "v1.8b", "v2.8b"],
        &[("v1", packed(8, &[0x7f])), ("v2", packed(8, &[0xf7]))],
        packed(8, &[0x00]),
    );
}

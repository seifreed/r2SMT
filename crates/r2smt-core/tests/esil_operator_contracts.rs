...
//!
//! Every string below is one radare2 emits or a minimal probe of one,
//! and every expected value was measured against the tool rather than
//! derived from this implementation. Originally taken on **6.1.8** and
//! re-probed on **6.2.0** — every operator pinned here answers
//! identically on both, which is worth recording because 6.2.0 *did*
//! change ESIL elsewhere (it fixed ARM32 `movt`, whose lowering this
//! project had attributed as a standing radare2 defect).
//!
//! The probe recipe is
//! `r2 -q -a x86 -b 64 -c aei -c 'ar rax=<v>' -c '"ae <esil>"' -c 'ar rax'`.
//! Two traps in it, both paid for: the `"…"` quoting is mandatory because `>`,
...
/// Which rung lifted this instruction. An `Unknown` in the output used
/// to separate the two rungs — the per-mnemonic x86 handler left
/// `OF`/`PF` free where radare2's ESIL spelled them out — but the
/// per-mnemonic integer handlers now model `CF`/`OF`/`PF` exactly, which
/// is the parity that removed that signal for `cmp`/`add`/`sub`. So the
/// rung is told apart structurally instead: the ESIL machine's masked
/// `old`/`cur` lowering is a different `Expr` tree from the per-mnemonic
/// handler's direct one, so the full lift equals the pure per-mnemonic
/// lift exactly when the per-mnemonic rung ran.
fn took_the_esil_rung(slice: &r2smt_slicer::Slice, arch: Arch, esil_flags: bool) -> bool {
    let full = r2smt_slicer::lift_slice_with(slice, arch, esil_flags);
    let per_mnemonic = r2smt_slicer::lift_per_mnemonic(&slice.instructions[0], arch);
    format!("{:?}", full.statements) != format!("{per_mnemonic:?}")
}
...
fn test_the_gate_off_sends_a_flag_setting_x86_instruction_to_its_handler() {
    ...
    assert!(!took_the_esil_rung(&slice, Arch::X86_64, false));
}
...
fn test_the_gate_on_lets_a_flag_setting_x86_instruction_take_the_esil_rung() {
    ...
    assert!(took_the_esil_rung(&slice, Arch::X86_64, true));
}

#[test]
fn test_the_gate_on_lets_a_flag_setting_arm_instruction_take_the_esil_rung() {
    // ARM was refused here regardless of the flag until 2026-08-10. Both
    // measured reasons expired (the carry convention fixed 2026-08-07,
    // radare2's a64 `subs` seeding fixed upstream in 6.2.0), and a corpus
    // verdict diff over the three ARM corpora showed opening only *adds*
    // correct actionable findings and loses none. The carry polarity the
    // old refusal stood in for — radare2's architectural `C` is this
    // pipeline's inverse — is proven sound directly and independently of
    // the gate by `test_the_arm_carry_is_stored_as_its_complement`.
    let slice = cmp_slice(
        "x1,x0,==,$z,zf,:=,63,$s,nf,:=,64,$b,!,cf,:=,63,$o,vf,:=",
        "cmp",
        &["x0", "x1"],
    );
    assert!(took_the_esil_rung(&slice, Arch::Aarch64, true));
}
...
#[test]
fn test_the_less_than_sign_extends_a_narrower_operand() {
    // `al,5,<` compares `5 < al` (second token is the LHS) with `al`
    // eight bits wide and the constant sixty-four. `al = 0xff` is -1, so
    // `5 < -1` is false. Zero-extending `al` to 64 bits (255) would flip
    // it to true — the signed compare must sign-extend the narrower side.
    assert_eq!(
        solve_esil("al,5,<,rbx,=", &[("rax", 0xff, 64)], ("rbx", QWORD), 0),
        SmtResult::AlwaysTrue
    );
}

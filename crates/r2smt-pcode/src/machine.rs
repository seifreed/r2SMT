...
impl Machine {
    ...
    fn eval(
        ...
        let bin = |m: &mut Self| -> Result<(Expr, Expr), PcodeError> {
            ...
            // A size-less constant takes its co-operand's width, not the
            // pointer width. An `INT_SRIGHT` amount or an `INT_SLESS`
            // immediate must share the register operand's architectural
            // width, or the encoder zero-extends the *value* past its sign
            // bit — a wrong arithmetic shift or signed compare. The output
            // width is the wrong hint here: a compare's output is one bit.
            // Falls back to the pointer width only when neither operand
            // fixes it.
            let pb = m.ptr_bits();
            let (wa, wb) = (m.operand_bits(a), m.operand_bits(b));
            Ok((
                m.read_operand(a, wb.unwrap_or(pb))?,
                m.read_operand(b, wa.unwrap_or(pb))?,
            ))
        };
    ...
    /// The operand's bit width when it is fixed by the varnode itself:
    /// `None` for a size-less constant (whose width a binary op must take
    /// from its co-operand), `Some` otherwise.
    fn operand_bits(&self, vn: &Varnode) -> Option<u16> {
        match vn {
            Varnode::Const { size: None, .. } => None,
            _ => Some(varnode_bits(vn, self.arch)),
        }
    }

    /// Read a varnode, sizing a size-less constant to `sizeless_const_bits`
    /// (its co-operand's width) instead of the pointer width.
    fn read_operand(&mut self, vn: &Varnode, sizeless_const_bits: u16) -> Result<Expr, PcodeError> {
        if let Varnode::Const { value, size: None } = vn {
            return Ok(Expr::konst(u128::from(*value), sizeless_const_bits));
        }
        self.read(vn)
    }
...
fn register_bits(r: &str, arch: Arch) -> u16 {
    // The GPR / stack / link / program-counter names carry the
    // architecture's pointer width: 32 on `Arch::Arm` (ARM32 `r0..r15`,
    // `sp`/`lr`/`pc` are 32-bit), 64 on `Aarch64` and `x86_64`. Minting
    // an ARM32 `r0` as a 64-bit varnode is a wrong *value*, not a lost
    // one — a signed compare of a 32-bit-negative value zero-extended
    // into 64 bits reads as positive. The `xN` / `wN` names are AArch64
    // only and keep their fixed 64 / 32.
    ...
    match r {
        ...
        _ if r.starts_with('r') && r[1..].chars().all(|c| c.is_ascii_digit()) => ptr,
        _ => ptr,
    }
}
...
0x100: mul w8, w8, w9
    ...
    #[test]
    fn size_less_shift_amount_takes_the_register_width_not_pointer_width() {
        // `asr w8, w9, #4` -> `INT_SRIGHT w9, 0x4`. The size-less shift
        // amount must be 32-bit (w9's width): at the 64-bit pointer width
        // the encoder zero-extends w9 to 64 and the arithmetic shift loses
        // its sign bit (`0x80000000 asr 4` gives 0x08000000, not 0xF8000000).
        let txt = "\
0x100: asr w8, w9, #4
    w8 = INT_SRIGHT w9, 0x4";
        let lift = lift_pcode(txt, Arch::Aarch64).unwrap();
        let IrStmt::Assign { src, .. } = &lift.statements[0] else {
            panic!("expected Assign");
        };
        assert_eq!(
            *src,
            Expr::ashr(Expr::Var(Var::new("w9", 32)), Expr::konst(4, 32))
        );
    }

    #[test]
    fn size_less_signed_compare_immediate_takes_the_register_width() {
        // `INT_SLESS w8, 0xffffffff`: the immediate must be 32-bit so it
        // reads as -1, not the 64-bit positive 0xffffffff. At the pointer
        // width the signed compare flips for w8 = 0.
        let txt = "\
0x100: cmp w8, w9
    (unique,0x10,1) = INT_SLESS w8, 0xffffffff";
        let lift = lift_pcode(txt, Arch::Aarch64).unwrap();
        let IrStmt::Assign { src, .. } = &lift.statements[0] else {
            panic!("expected Assign");
        };
        assert_eq!(
            *src,
            Expr::slt(Expr::Var(Var::new("w8", 32)), Expr::konst(0xffff_ffff, 32))
        );
    }

    #[test]
    fn arm32_gpr_is_32_bit_not_defaulted_to_64() {
        // On `Arch::Arm`, `r0..r15` / `sp` / `lr` / `pc` are 32-bit.
        // Minting them at the old hardcoded 64 is a wrong signed value:
        // a 32-bit-negative operand zero-extended to 64 reads as
        // positive under a later signed compare.
        let txt = "\
0x100: add r0, r0, r1
    r0 = INT_ADD r0, r1";
        let lift = lift_pcode(txt, Arch::Arm).unwrap();
        let IrStmt::Assign { dst, src } = &lift.statements[0] else {
            panic!("expected Assign");
        };
        assert_eq!(dst.bits, 32);
        assert_eq!(
            *src,
            Expr::add(Expr::Var(Var::new("r0", 32)), Expr::Var(Var::new("r1", 32)))
        );
    }

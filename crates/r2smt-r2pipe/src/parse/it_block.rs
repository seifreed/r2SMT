...
/// `bics`/`adcs`/`sbcs` in `cs`, `lsls`/`muls`/`mls`/`vmls`/`smmls`/
/// `vnmls`/`smlals`/`umlals` in `ls`, `vcge`/`vcgt`/`vcle` in
/// `ge`/`gt`/`le`. Rather than guess, the fold is abandoned when one is
/// covered by an IT block.
///
/// This is a *denylist*, so a missing entry is unsound (a predicated
/// instruction left unconditional — a fabricated verdict), while a
/// spurious entry is merely a lost fold. The ARM mnemonic set is fixed,
/// so completeness is stable rather than version-dependent; every real
/// Thumb-2 mnemonic ending in a two-letter condition code belongs here.
const COND_AMBIGUOUS_MNEMONICS: &[&str] = &[
    "teq", "movs", "bics", "adcs", "sbcs", "lsls", "muls", "mls", "vmls", "smmls", "vnmls",
    "smlals", "umlals", "vcge", "vcgt", "vcle",
];
...
mod tests {
    ...
    #[test]
    fn test_it_block_abandons_the_fold_on_smmls_which_ends_in_a_condition_code() {
        // `smmls` (a real Thumb-2 multiply-subtract) ends in `ls`.
        // Treating that as an existing `ls` suffix would leave the
        // instruction unconditional inside `it ls` — a fabricated verdict.
        // It must abandon the fold and leave the `it` for the slicer.
        let mut blocks = block(vec![
            insn(0, "it", &["ls"]),
            insn(2, "smmls", &["r0", "r1", "r2", "r3"]),
        ]);
        fold_it_blocks(&mut blocks);
        assert_eq!(mnemonics(&blocks), vec!["it", "smmls"]);
    }

...
pub struct LocalVariable {
    ...
    /// (`stk_rbp_-4`, `stk_rsp_8`) — `format!("stk_{base}_{offset}")` with
    /// a signed `offset`, so a positive slot carries no `+` sign (the
    /// lifter's consumer in `effect.rs` formats it identically).

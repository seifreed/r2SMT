...
#[test]
fn an_unsizable_finding_is_skipped_not_fatal_to_the_plan() {
    // A finding with no resolved fallthrough cannot be sized. It must be
    // skipped like every other rejection, leaving the rest of the plan
    // intact — not propagated as an error that discards all operations.
    let mut unsizable = finding(
        SmtResult::AlwaysFalse,
        FindingKind::DeadBranch,
        Confidence::High,
        "jne",
    ...
    unsizable.fallthrough_target = None;
    let good = finding(
        SmtResult::AlwaysFalse,
        FindingKind::DeadBranch,
        Confidence::High,
    ...
    .unwrap();
    assert_eq!(plan.operations.len(), 1, "the sizable finding is planned");
    assert_eq!(plan.operations[0].size, 2);
    assert_eq!(plan.skipped.len(), 1, "the unsizable finding is skipped");
    assert_eq!(plan.skipped[0].0, Address(0x40_1060));
}

...
pub enum EsilToken {
    ...
    /// pushes one. The payload is the canonical operator string, one of
    /// `"+"`, `"-"`, `"*"`, `"/"`, `"%"`, `"&"`, `"|"`, `"^"`, `"<<"`,
    /// `">>"`, `"<"`, `"<="`, `">"`, `">="`.
    ///
    /// Deliberately absent are radare2's arithmetic-shift-right (`>>>>`)
    /// and rotates (`>>>` / `<<<`): they tokenize as [`Unknown`] so the
    /// instruction declines to the per-mnemonic handler. That is the
    /// sound choice, not an oversight — the machine's `>>` is a raw
    /// bit-vector shift matching radare2's VM, which does *not* mask the
    /// count (the documented `shr r32, cl` gap), so lifting `sar` the
    /// same way would inherit that wrong value where the per-mnemonic
    /// handler masks it correctly. Adding them soundly means first
    /// deciding the pending shift-count-masking question for the whole
    /// family.
    ///
    /// [`Unknown`]: EsilToken::Unknown
    Binary(&'static str),
